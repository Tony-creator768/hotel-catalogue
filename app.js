...
function hasUnitLink(row) {
  return Boolean(firstLink(row));
}
...
function mediaHtml(item) {
  const image = String(item.image || firstNonEmpty(item.rows || [], "Image URL") || firstNonEmpty(item.rows || [], "Unit Image URL") || "").trim();
...
function unitRowHtml(row, compact = false) {
  ...
  const content = `
    <div>
      ...
      <span>${escapeHtml(bedroomLabel(get(row, "Bedrooms")))}${get(row, "Size SQM") ? ` • ${escapeHtml(get(row, "Size SQM"))} sqm` : ""}</span>
    </div>
    <div class="unit-side">
...
function renderCards(groups) {
  ...
  els.grid.innerHTML = groups.map(group => {
    ...
    const allApprovedCount = group.allRows.length;
    return `
      <article class="card hotel-card">
        ...
        <div class="card-body">
          ...
          <div class="unit">${escapeHtml(group.area || "Area not listed")}${group.address ? ` • ${escapeHtml(group.address)}` : ""}</div>
          <div class="meta">
            <span class="pill">${group.unitCount} unit option${group.unitCount === 1 ? "" : "s"}</span>
            <span class="pill">${escapeHtml(bedroomSummary)}</span>
          ...
          <div class="card-actions">
            ...
            <button class="${isSelected ? "compare-selected" : "ghost"}" onclick="toggleCompare('${escapeHtml(id)}')">${isSelected ? "Selected" : "Compare"}</button>
          </div>
        </div>
      </article>
    `;
      ...
      <div class="modal-content">
        ...
        <div class="details-grid">
          <div class="detail-block">
            <h4>Rate range</h4>
            <p>${rateRange(group)} per month\n${rowsToShow.length} approved bedroom/unit option${rowsToShow.length === 1 ? "" : "s"}</p>
          </div>
          <div class="detail-block">
            <h4>Location</h4>
...
function exportCsv() {
  const headers = ["Property Name", "Category", "Area", "Address", "Unit Type", "Bedrooms", "Monthly Rate QAR", "Pet Friendly", "Contact Email", "Website URL", "Unit Detail URL", "Unit Video URL"];
